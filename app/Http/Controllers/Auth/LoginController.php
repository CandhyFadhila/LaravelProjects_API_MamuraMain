...
{
    // public function signIn(LoginDashboardRequest $request)
    // {
    ...
    //     $filteredUser = $user->makeHidden(['password', 'remember_token', 'roles', 'documents']);
    ...
    public function signInAdmin(LoginDashboardRequest $request)
    {
        return $this->signInWithContext($request, 'admin', 'Super Admin', 'admin');
    }

    public function signInUser(LoginDashboardRequest $request)
    {
        return $this->signInWithContext($request, 'user', 'Users', 'user');
    }
    ...
    private function signInWithContext(LoginDashboardRequest $request, string $context, string $requiredRole, string $ability)
    {
        $credential = $request->validated();

        // (Opsional) hindari session: manual check biar murni API
        $user = \App\Models\User::where('email', $credential['email'])->first();
        ...
        if (! $user->hasRole($requiredRole)) {
            Log::channel('auth_login')->info("| Login | - Forbidden role for email: {$credential['email']} on context: {$context}");
            return response()->json(
                new WithoutDataResource(
        ...
        // (Opsional) single-session policy: revoke token lama
        // $user->tokens()->delete();

        $user->update(['last_login' => now()]);
        Log::channel('auth_login')->info("| Login | - Login success for email: {$credential['email']}, at {$user->last_login}");

        // === KUNCI: token dengan ability spesifik ===
        $plainToken = $user->createToken("{$context}_token_" . Str::uuid(), [$ability])->plainTextToken;
        ...
        return response()->json(
            new WithDataResource(
                ...
                'LOGIN_SUCCESS',
                'Login Berhasil',
                'Selamat datang ' . $user->name . ', anda berhasil login kedalam Mamura!.',
                [
                    'token' => $plainToken,
                    'user' => $filteredUser,
                    'role' => $filteredRoles,
